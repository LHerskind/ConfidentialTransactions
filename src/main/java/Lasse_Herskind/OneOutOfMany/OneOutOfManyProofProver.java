...
public class OneOutOfManyProofProver<T extends GroupElement<T>> implements Prover<DoubleGeneratorParams<T>, T[], OneOutOfManyWitness<T>, OneOutOfManyProof<T>> {
    @Override
    public OneOutOfManyProof<T> generateProof(DoubleGeneratorParams<T> parameter, T[] CMList, OneOutOfManyWitness<T> witness) {
        ...
        indexesK.parallelStream().forEach(k -> {
            ...
            BigInteger gamma = BigInteger.ZERO; //ProofUtils.randomNumber();
        ...
        BigInteger zV = witness.getV().multiply(x.pow(m));
        ...
        return new OneOutOfManyProof<>(A, B, C, D, Gks, Qks, fs, zA, zC, zV, zR);
    }
    ...
    private static BigInteger f(int[] l_bin, BigInteger[][] a, int j, int i, BigInteger x) {
        if(i == 0){
         //   return sigma(l_bin[j], i).multiply(x).subtract(a[j][i]);
        }
