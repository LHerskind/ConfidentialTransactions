...
public class Spending_Two_Clean_MW_V5 {
    ...
    public static void main(String[] args) throws VerificationFailedException {
        ...
        DoubleBlindedPedersenCommitment output_coin1 = LelantusUtils.getDBPedersen(params, BigInteger.ZERO, V1, ProofUtils.randomNumber());
        System.out.println("coin_extracted: \t" + output_coin1.getCommitment().stringRepresentation());
        DoubleBlindedPedersenCommitment output_coin2 = LelantusUtils.getDBPedersen(params, BigInteger.ZERO, V2, ProofUtils.randomNumber());
        System.out.println("coin_extracted: \t" + output_coin2.getCommitment().stringRepresentation());
        ...
        DoubleBlindedPedersenCommitment serialNumber1 = LelantusUtils.getDBPedersen(params, S1, BigInteger.ZERO, BigInteger.ZERO);// .add(output_coin1);
        DoubleBlindedPedersenCommitment serialNumber2 = LelantusUtils.getDBPedersen(params, S2, BigInteger.ZERO, BigInteger.ZERO);// .add(output_coin2);
        ...
        OneOutOfManyWitness oneOutOfManyWitness1 = new OneOutOfManyWitness(N-1, shielded_coin1);
        ...
        OneOutOfManyWitness oneOutOfManyWitness2 = new OneOutOfManyWitness(N-1, shielded_coin2);
        ...
        GroupElement public_A = LelantusUtils.getAPublic(params, oneOutOfManyProof1, oneOutOfManyProof2);

        System.out.println(public_A);
        BigInteger R = ProofUtils.randomNumber();
        DoubleBlindedPedersenCommitment output_coin = LelantusUtils.getDBPedersen(params, BigInteger.ZERO, V1.add(V2), R);

        BigInteger x = LelantusUtils.getChallenge(params, oneOutOfManyProof1);

        GroupElement divides_A = output_coin.getCommitment().multiply(x.pow(m));
        BigInteger offset = (R).multiply(x.pow(m));

        public_A = divides_A.subtract(public_A); // The excess! We can use public_a as the input! cool!
        System.out.println(public_A);
        private_A = offset.subtract(private_A);
