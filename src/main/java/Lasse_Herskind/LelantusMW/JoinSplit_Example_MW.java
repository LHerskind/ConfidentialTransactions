...
public class JoinSplit_Example_MW {
    ...
    public static void main(String[] args) throws VerificationFailedException {
        ...
        // And now the transperant one!
        BigInteger V_t = BigInteger.valueOf(5);
        BigInteger R_t = ProofUtils.randomNumber();
        DoubleBlindedPedersenCommitment coin_t = LelantusUtils.getDBPedersen(params, BigInteger.ZERO, V_t, R_t);
        ...
        System.out.println("coin_t: \t\t\t" + coin_t.getCommitment().stringRepresentation());
        ...
        GroupElement coin_t_no_r = LelantusUtils.getDBPedersen(params, coin_t.getSerial(), coin_t.getValue(), BigInteger.ZERO).getCommitment();
        GroupElement A = (coin_o1.add(coin_o2).add(coin_o3).getCommitment()).multiply(x.pow(m)).subtract(coin_t.getCommitment().multiply(x.pow(m)));
